...
async function gettingDpStatus(uID: number): Promise<User | null> {
    return new Promise((resolve, reject) => {
        db.query("SELECT * from profilephoto where uID=?", [uID], (error: Error, results: Array<User>) => {
            if (error) {
                reject (error);
            }
            else {
...
export async function POST(request: Request) {
    try {
        ...
        const isDP = await gettingDpStatus(uID);
        if (isDP)
            db.query("UPDATE profilephoto set photoUrl=? where uID=?", [uploadResult.secure_url, uID]);

        else {
            db.query("INSERT INTO profilephoto(uID, photoUrl) VALUES(?, ?)", [uID, uploadResult.secure_url]);
        }
